...
def show_map_and_list(radius, df_filtered):
    ...
    with list_col:
        ...
        for _, row in hospitals_to_show.iterrows():
            lat = row["lat"]
            lon = row["lon"]
            naver = f"https://map.naver.com/v5/search/{lat},{lon}"
            ...
            <div style="background-color:white;padding:10px;border-radius:10px;margin-bottom:8px;">
                ...
                거리: {row['distance']:.2f} km<br>
                🔗 <a href="{naver}" target="_blank">네이버</a> |
                <a href="{kakao}" target="_blank">카카오</a> |
                <a href="{google}" target="_blank">구글</a>
                </span>
            </div>
            """, unsafe_allow_html=True)
